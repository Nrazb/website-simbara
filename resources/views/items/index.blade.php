...
@section('content')
    ...
    <div class="p-4 sm:p-6 bg-white rounded-2xl shadow-sm" x-data="{
        selectedItem: null,
        ...
        openModal(type, item) {
            ...
            this.openMaintenance = type === 'maintenance';
            this.openMutation = type === 'mutation';
            this.openRemove = type === 'remove';
        },
        closeAll() {
            this.openMaintenance = false;
            this.openMutation = false;
            this.openRemove = false;
            this.selectedItem = null;
        }
    }" x-init="maintenanceUnits = {{ Js::from($maintenanceUnits ?? []) }};
    unitsSelect = {{ Js::from($unitsSelect ?? []) }}">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-5">
            ...
            <div class="flex items-center gap-2">
                <button onclick="window.location.href='{{ route('items.create') }}'"
                    class="bg-blue-900 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center space-x-1">
                    <i class="fa-solid fa-plus"></i>
                    <span>Tambah Barang</span>
                </button>
                <form id="importForm" action="{{ route('items.import') }}" method="POST" enctype="multipart/form-data"
                    class="border border-gray-300 text-gray-700 rounded-lg text-sm font-medium flex items-center space-x-1">
                    @csrf
                    <input id="importInput" type="file" name="file" accept=".xlsx,.xls,.csv" class="hidden">
                    <button type="button" id="importButton"
                        class="px-4 py-2 hover:bg-gray-100 flex items-center space-x-1">
        ...
        <div class="mb-4 rounded-xl shadow-sm">
            <form method="GET" class="flex flex-wrap items-start gap-4" x-data x-ref="filterForm">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="relative min-w-[150px]">
                                ...
                                <select name="year" x-ref="year"
                                    @change="$refs.startDate && ($refs.startDate.value=''); $refs.endDate && ($refs.endDate.value=''); $refs.filterForm.submit()"
                                    class="border rounded-lg pl-9 pr-8 py-2 bg-white shadow-sm focus:ring-2 focus:ring-blue-400 focus:border-blue-500 transition w-full text-sm">
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="flex flex-col">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div class="relative min-w-[150px]">
                                    <input type="date" name="end_date" value="{{ request('end_date') }}" x-ref="endDate"
                                        @change="$refs.year && ($refs.year.value=''); $refs.filterForm.submit()"
                        ...
                        <div class="flex flex-col">
                            <label class="text-gray-500 text-sm font-medium">Rentang Nilai</label>
                            <div class="flex items-center gap-2">
                                <div class="relative min-w-[150px]">
                                    <i
                                        class="fa-solid fa-money-bill absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                                    <input type="number" name="min_cost" value="{{ request('min_cost') }}"
                                        placeholder="Min Nilai" @change="$refs.filterForm.submit()"
                                        class="border rounded-lg pl-9 py-2 bg-white shadow-sm text-sm focus:ring-2 focus:ring-blue-400" />
                                ...
                                <div class="relative min-w-[150px]">
                                    <i
                                        class="fa-solid fa-money-bill absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                                    <input type="number" name="max_cost" value="{{ request('max_cost') }}"
                                        placeholder="Max Nilai" @change="$refs.filterForm.submit()"
                                        class="border rounded-lg pl-9 py-2 bg-white shadow-sm text-sm focus:ring-2 focus:ring-blue-400" />
                                </div>
                            </div>
                        </div>
                ...
                @if (request()->anyFilled(['user_id', 'year', 'start_date', 'end_date', 'min_cost', 'max_cost']))
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($items as $item)
                        <tr class="hover:bg-gray-50 transition" data-id="{{ $item->id }}">
                            ...
                            <td class="px-4 py-3" data-type-id="{{ $item->type?->id }}">{{ $item->type?->name ?? '—' }}
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button type="button"
                                        ...
                                        title="Pemeliharaan"
                                        @click="openModal('maintenance', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
                                        <i class="fa-solid fa-wrench"></i>
                                    </button>
                                    <button type="button"
                                        class="px-3 py-1 rounded-lg bg-blue-900 text-white hover:bg-blue-700"
                                        title="Mutasi"
                                        @click="openModal('mutation', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
                                        <i class="fa-solid fa-exchange-alt"></i>
                                    </button>
                                    <button type="button"
                                        class="px-3 py-1 rounded-lg bg-red-600 text-white hover:bg-red-700" title="Hapus"
                                        @click="openModal('remove', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
            ...
            @foreach ($items as $item)
                <div class="border border-gray-200 rounded-xl p-3 shadow-sm" data-id="{{ $item->id }}">
                    ...
                    <div class="mt-2 text-sm text-gray-600 space-y-1">
                        ...
                        <p><span class="font-medium" data-type-id="{{ $item->type?->id }}">Jenis:</span>
                            {{ $item->type?->name ?? '—' }}</p>
                        ...
                        <p><span class="font-medium">Nilai BMN:</span> Rp {{ number_format($item->cost, 0, ',', '.') }}
                        </p>
                        <div class="flex items-center gap-2 pt-2">
                            <button type="button" class="px-3 py-1 rounded-lg bg-amber-500 text-white hover:bg-amber-600"
                                title="Pemeliharaan"
                                @click="openModal('maintenance', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
                                <i class="fa-solid fa-wrench"></i>
                            </button>
                            <button type="button" class="px-3 py-1 rounded-lg bg-blue-900 text-white hover:bg-blue-700"
                                title="Mutasi"
                                @click="openModal('mutation', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
                                <i class="fa-solid fa-exchange-alt"></i>
                            </button>
                            <button type="button" class="px-3 py-1 rounded-lg bg-red-600 text-white hover:bg-red-700"
                                title="Hapus"
                                @click="openModal('remove', {id:'{{ $item->id }}', name:'{{ $item->name }}'})">
                                <i class="fa-solid fa-trash"></i>
                            </button>
        ...
        <div x-cloak x-show="openMaintenance && selectedItem" x-transition
            class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4"
            @click.self="closeAll()">
            <div class="bg-white w-full max-w-xl rounded-2xl shadow-lg p-6">
                <h2 class="text-lg font-semibold mb-4">Permintaan Pemeliharaan</h2>
                <form method="POST" action="{{ route('maintenance-item-requests.store') }}">
                    @csrf
                    <input type="hidden" name="item_id" :value="selectedItem?.id">
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Unit Pemeliharaan</label>
                        <select name="maintenance_user_id" class="w-full border rounded-lg px-3 py-2 text-sm">
                            <template x-for="u in maintenanceUnits" :key="u.id">
                                <option :value="u.id" x-text="u.name"></option>
                            </template>
                        </select>
                    </div>
                    <div class="flex justify-end gap-2">
                        <button type="button" class="px-4 py-2 rounded-lg border" @click="closeAll()">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-amber-500 text-white hover:bg-amber-600">Ajukan</button>
                    </div>
                </form>
            </div>
        </div>

        <div x-cloak x-show="openMutation && selectedItem" x-transition
            class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4"
            @click.self="closeAll()">
            <div class="bg-white w-full max-w-xl rounded-2xl shadow-lg p-6">
                <h2 class="text-lg font-semibold mb-4">Permintaan Mutasi</h2>
                <form method="POST" action="{{ route('mutation-item-requests.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="to_user_id" class="w-full border rounded-lg px-3 py-2 text-sm">
                            <template x-for="u in unitsSelect" :key="u.id">
                                <option :value="u.id" x-text="u.name"></option>
                            </template>
                        </select>
                    </div>
                    <div class="flex justify-end gap-2">
                        <button type="button" class="px-4 py-2 rounded-lg border" @click="closeAll()">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-blue-900 text-white hover:bg-blue-700">Ajukan</button>
                    </div>
                </form>
            </div>
        </div>

        <div x-cloak x-show="openRemove && selectedItem" x-transition
            class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4"
            @click.self="closeAll()">
            <div class="bg-white w-full max-w-xl rounded-2xl shadow-lg p-6">
                <h2 class="text-lg font-semibold mb-4">Pengajuan Penghapusan?</h2>
                <form method="POST" action="{{ route('remove-item-requests.store') }}">
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" class="px-4 py-2 rounded-lg border" @click="closeAll()">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">Ajukan</button>
                    </div>
                </form>
            </div>
        </div>

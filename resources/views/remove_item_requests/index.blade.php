...
<div class="p-4 sm:p-6 bg-white rounded-2xl shadow-sm">
    <div class="flex flex-wrap items-center mb-4 gap-4">
        <form method="GET" class="flex flex-wrap items-center gap-3">
            ...
            @if(request()->anyFilled(['search','user_id','status','unit_confirmed']))
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3">
                        @if(auth()->user()->role === 'ADMIN')
                            <div x-data="{ open: false }" class="relative inline-block">
                                ...
                                <div x-show="open" x-transition class="absolute left-0 mt-1 bg-white border rounded shadow-md z-10">
                                    <form method="POST" action="{{ route('remove-item-requests.update-status', $data->id) }}">
                                        @csrf
                                        <input type="hidden" name="status" value="PROCESS">
                                        <button type="submit" class="w-full text-left px-3 py-2 hover:bg-gray-100">PROCESS</button>
                                    </form>
                                    <form method="POST" action="{{ route('remove-item-requests.update-status', $data->id) }}">
                                        @csrf
                                        <input type="hidden" name="status" value="STORED">
                                        <button type="submit" class="w-full text-left px-3 py-2 hover:bg-gray-100">STORED</button>
                                    </form>
                                    <form method="POST" action="{{ route('remove-item-requests.update-status', $data->id) }}">
                                        @csrf
                                        <input type="hidden" name="status" value="AUCTIONED">
                                        <button type="submit" class="w-full text-left px-3 py-2 hover:bg-gray-100">AUCTIONED</button>
                                    </form>
                                </div>
                            </div>
                        @else
                    ...
                    <td class="px-4 py-3 text-left">
                        @if(auth()->user()->id === $data->user_id && !$data->unit_confirmed)
                            <form method="POST" action="{{ route('remove-item-requests.confirm-unit', $data->id) }}">
                                @csrf
                                <button type="submit" class="px-3 py-1 rounded bg-blue-600 text-white text-sm hover:bg-blue-700">Konfirmasi</button>
                            </form>
                        @else
                            <span class="px-2 py-1 rounded text-white {{ $data->unit_confirmed ? 'bg-green-600' : 'bg-red-600' }}">
...
<template x-if="removeOpen">
    <div class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center overflow-y-auto p-2 sm:p-4" x-transition @click.self="removeOpen=false">
        <div class="bg-white w-[90%] sm:w-[80%] md:w-full max-w-xl rounded-xl shadow-lg p-6 relative" x-transition.opacity x-transition.scale>
            ...
            <form method="POST" action="{{ route('remove-item-requests.store') }}">
                ...
                <div class="flex flex-col gap-3">
                    <div>
                        ...
                        <select name="item_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm md:text-base focus:ring-blue-900 focus:border-blue-900" required>
                ...
                <div class="flex justify-center gap-3 border-t pt-4 mt-4">
                    <button type="button" class="px-6 py-2 rounded-lg border border-blue-900 text-gray-700 hover:bg-gray-200" @click="removeOpen=false">Batal</button>
                    <button type="submit" class="px-6 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700">Hapus</button>
                </div>
            </form>
        </div>
    </div>
